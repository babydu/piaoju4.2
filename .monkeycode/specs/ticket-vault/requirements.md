...
9. THE 系统 SHALL 对未填写到期日期的票据不进行任何形式的提醒
10. THE 系统 SHALL 提供「设为隐私票据」开关默认关闭
...
4. WHERE 用户未解锁隐私箱 THEN 系统 SHALL 仅搜索普通票据
5. WHERE 用户已解锁隐私箱 THEN 系统 SHALL 提供选项允许用户选择是否包含隐私票据
...
6. THE 备份内容 SHALL 包含Isar数据库文件、票据图片、隐私票据加密数据和用户配置
