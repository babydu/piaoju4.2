...
- **到期日期(Due Date)**：票据的有效期或到期时间，用于预留提醒功能
...
16. WHEN 用户选择「加入隐私箱」开关 THEN 系统 SHALL 保存时将票据直接存入隐私箱
...
6. WHERE 用户授权位置权限 THEN 系统 SHALL 提供位置选择器允许自动获取当前位置
7. THE 系统 SHALL 提供日期选择器默认填充当前时间且允许用户修改
8. THE 系统 SHALL 提供到期日期选择器允许用户选择性地填写票据有效期
9. THE 系统 SHALL 提供「加入隐私箱」开关默认关闭
...
4. THE 系统 SHALL 提供排序切换按钮允许用户按时间升序/降序或按名称排序
5. THE 系统 SHALL 采用分页懒加载方式展示票据列表每页10条
6. THE 系统 SHALL 在票据卡片中显示缩略图、标题、日期、标签和隐私标识
7. WHEN 用户下拉刷新 THEN 系统 SHALL 刷新票据列表
8. WHEN 用户上拉加载 THEN 系统 SHALL 加载更多票据
9. WHEN 用户点击票据卡片 THEN 系统 SHALL 导航至票据详情页
10. WHEN 用户长按任意票据卡片 THEN 系统 SHALL 进入多选模式
...
2. THE 系统 SHALL 在操作栏中仅显示「批量加入隐私箱」和「批量删除」两个功能按钮
3. WHEN 用户点击「批量加入隐私箱」 THEN 系统 SHALL 弹窗要求用户二次确认
...
4. THE 系统 SHALL 在更多操作中提供移入/移出隐私箱功能
...
5. THE 系统 SHALL 提供查看合集中票据列表的功能
6. THE 系统 SHALL 在首页筛选菜单中提供按合集筛选票据的功能

### 需求7：隐私箱功能

**用户故事：** AS 一个注重隐私的用户，我想要将敏感票据存储在加密区域，以便安全地管理我的私密票据。

#### Acceptance Criteria

1. WHEN 用户从侧边抽屉菜单点击「隐私箱」 THEN 系统 SHALL 要求用户输入密码或使用生物识别解锁
2. THE 系统 SHALL 支持6位数字密码和复杂密码两种方式
3. THE 系统 SHALL 将密码哈希存储于系统安全硬件
4. THE 系统 SHALL 采用物理隔离方案存储隐私箱数据
5. THE 系统 SHALL 使用独立Isar加密数据库存储隐私箱元数据
6. THE 系统 SHALL 使用AES-256-GCM逐文件加密隐私箱图片
7. THE 系统 SHALL 将加密密钥仅存储于系统安全硬件
8. THE 系统 SHALL 在用户锁屏或退出APP后自动销毁内存中的密钥
9. THE 系统 SHALL 默认禁止云端同步隐私箱数据
10. WHERE 用户手动开启隐私箱云同步 THEN 系统 SHALL 仅上传加密密文
11. WHEN 用户退出隐私箱页面 OR 退出APP THEN 系统 SHALL 自动锁定隐私箱
12. THE 隐私箱界面布局 SHALL 与首页保持一致支持筛选、搜索和排序
...
1. THE 系统 SHALL 使用Isar数据库存储结构化数据
2. THE 系统 SHALL 在应用沙盒中分目录存储普通票据和隐私箱票据
3. THE 系统 SHALL 使用WebP格式压缩存储票据图片
4. THE 系统 SHALL 对隐私箱数据强制加密存储
...
2. THE 系统 SHALL 支持自动周期备份功能仅在Wi-Fi和充电状态执行
3. THE 系统 SHALL 将备份数据保存为AES-256加密ZIP压缩包文件后缀为.pmi.enc
4. THE 系统 SHALL 要求用户在备份时设置自定义密码
5. WHEN 用户选择恢复备份 THEN 系统 SHALL 要求用户输入密码
6. WHEN 用户确认恢复 THEN 系统 SHALL 覆盖本地现有数据前要求二次确认
7. THE 备份内容 SHALL 包含Isar数据库文件、票据图片、隐私箱加密数据和用户配置

### 需求13：云端同步功能

**用户故事：** AS 一个有多设备需求的用户，我想要将票据同步到云端，以便在不同设备间访问我的票据。
...
4. THE 系统 SHALL 仅申请生物识别权限用于隐私箱解锁且默认关闭
...
3. THE 侧边抽屉菜单 SHALL 依次显示首页、合集管理、隐私箱、设置、帮助与反馈
